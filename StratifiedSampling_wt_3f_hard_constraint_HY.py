...
HY = pd.read_csv('HY_UPDATED_7-1.csv')
base = pd.read_csv('BaselineHY.csv')
baseAvg = pd.read_csv("HY_Index_Averages_7-8.csv")

'''
Clearn up data: add Date column
'''
HY['Date'] = pd.to_datetime(HY['prd_formatted']).dt.to_period('D')
...
'''
HY_by_month = a list of dataframes of each month
'''
HY_by_month = []
for _, g in HY.groupby(HY['Date']):
    HY_by_month.append(g)
...
'''
getPorfolioActual:
    Input: data = HY_by_month, tt_portfolio = the desired portfolio (output from
...
def ss(df, base, f1, f2, f3, numBond, numBucket, turnover): 
    ...
    writeFile('Results_wt_HY_%.2f.txt' %(turnover), contents)
    return Port1A, Error1, Turnover1

#calling main function
Port, Error, Turnover = ss(HY_by_month, base, 'coupon', 'oad', 'oas', 11, 27, 0.04)

#Writing output files: 
Error.to_csv('TrackingError_wt_HY.csv')
Turnover.to_csv('Turnover_wt_HY.csv')
pd.concat(Port).to_csv('Portfolio_wt_HY.csv')
...
for y, g in Error.groupby(Error['Year']):
        ret_avg[y]=(g['Return'].mean())*12
        ret_avg_b[y]=(g['benchmark'].mean())*12

OAD.to_csv('OAD_wt_HY.csv')
OAS.to_csv('OAS_wt_HY.csv')
Rating.to_csv('Rating_wt_HY.csv')
MV.to_csv('MV_wt_HY.csv')   
...
annualRes.to_csv('Annual_Results_wt_HY.csv')   
