...
def buckets(sector, HY, start_yr, num_of_yrs):
    HY_yr = [] # a list of dataframes for each year
    ...
    for i in range(num_of_yrs):
        year = start_yr+i
        HY_cur_yr = HY.loc['%d' %year, ['class_2', 'cusip', 'total_return_volatility', 
                                 'market_value', 'prev_6m_total_return', 'coupon', 
                                 'oas', 'oad', 'total_return_mtd', 'sp_rating_number']]
        HY_yr.append(HY_cur_yr)
        sector_cur_yr = HY_cur_yr.loc[HY_cur_yr['class_2'] == sector]
...
# total_num is the total number of bonds we want to be in monthly portfolio
    for j in range(len(sector_mon)):
        ...
        for factor in list(D):
            col = D[factor]
            sector_mon[j][factor] = (col - col.mean()) / col.std() 
...
def get_portfolio(cluster_map, n, index_mv, total_num, to_rate):    
    ...
    for i in range(1, len(cluster_map)): #access month
        ...
        cur_index_mv = index_mv[i]
        # get expected portfolio
        cur_mv_wt, new_pf_mv, new_pf_to = get_exp_portfolio(curmap, prev_pf, n,  
                                          cur_index_mv, total_num, to_rate)
        mv_wt[i] = cur_mv_wt
        new_pf = new_pf_to
...
def turnover_rate(x,y):
    # x is the previous month's portfolio
    # y is the current month's portfolio
    # get a list of all the bonds in prev and cur month
    a = set(x.loc[:, 'cusip'].tolist())
    b = set(y.loc[:, 'cusip'].tolist()) #current month bonds
    all_bonds = list(a.union(b)) 
    
    prev = x.set_index('cusip')
    cur = y.set_index('cusip')
    
    # compute the sum of absolute value of weight differences
    turnover_1 = 0
    for cusip in all_bonds:
        if prev.index.contains(cusip) and (not cur.index.contains(cusip)):
            turnover_1 += 1 
        elif (not prev.index.contains(cusip)) and cur.index.contains(cusip): 
            turnover_1 += 1 
        
    return turnover_1
       
def compute_to(pflist):
    turnover_1 = []
    for i in range(len(pflist)-1):
        to1 = turnover_rate(pflist[i], pflist[i+1])
        turnover_1.append(to1)
